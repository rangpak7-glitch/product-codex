...
GitHub Actions 워크플로는 매일 한국 시간 오전 4시(UTC 19:00)에 `data/dailyContents.js`에 말씀 붙들기, 감사기도, 저녁기도, 아침기도, 큐티(QT) 콘텐츠를 생성합니다. `main`에 변경을 푸시하면 Cloudflare Pages가 기존 Git 연동을 통해 배포합니다.
...
워크플로는 **Actions → Generate daily content → Run workflow**에서 수동 실행할 수 있습니다. 같은 한국 날짜의 5개 항목이 이미 모두 있으면 콘텐츠를 다시 만들거나 커밋하지 않습니다.
...
`.github/workflows/youtube-sync.yml`은 일일 콘텐츠 갱신과 같은 한국 시간 오전 4시에 실행됩니다. 채널의 공식 RSS와 공개 재생목록 RSS를 사용하므로 별도의 `YOUTUBE_API_KEY` 설정은 필요하지 않습니다. 일일 콘텐츠 워크플로와 같은 동시 실행 그룹을 사용해 두 작업의 `main` 푸시가 겹치지 않게 처리합니다.

동기화 스크립트는 YouTube의 제목과 정확한 게시 시각을 반영하되, 같은 영상 ID에 이미 지정한 관련 말씀, 주제, 태그, 설명은 수동 보정값으로 보존합니다. `아침기도`, `저녁기도`, `하루 한 말씀, 인생 회복` 재생목록의 합집합만 Shorts로 분류합니다. RSS 확인이 실패하거나 대상 재생목록이 비어 있으면 기존 `data/videos.js`를 유지하고 커밋하지 않습니다. `videos.html`은 전체 업로드를 최신순으로 보여 주고 `동영상`과 `Shorts`로 나눕니다. 채널 일반 게시물은 공개 RSS 수집 대상이 아니므로 관리자 화면의 `채널 소식 등록`에서 별도로 관리합니다.
