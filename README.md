...
`.github/workflows/youtube-sync.yml`은 일일 콘텐츠 갱신과 같은 한국 시간 오전 4시에 실행됩니다. 채널의 공식 RSS와 공개 Shorts 탭을 사용하므로 별도의 `YOUTUBE_API_KEY` 설정은 필요하지 않습니다. 일일 콘텐츠 워크플로와 같은 동시 실행 그룹을 사용해 두 작업의 `main` 푸시가 겹치지 않게 처리합니다.

동기화 스크립트는 YouTube의 제목과 게시일, Shorts 여부를 반영하되, 같은 영상 ID에 이미 지정한 관련 말씀, 주제, 태그, 설명은 수동 보정값으로 보존합니다. RSS 또는 Shorts 확인이 실패하거나 공개 영상을 찾지 못하면 기존 `data/videos.js`를 유지하고 커밋하지 않습니다. `videos.html`의 `Shorts` 필터는 `isShort`가 확인된 채널 영상만 보여 줍니다. 채널 일반 게시물은 공개 RSS 수집 대상이 아니므로 관리자 화면의 `채널 소식 등록`에서 별도로 관리합니다.
